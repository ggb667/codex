...
mod tests {
    ...
    fn parse_hex_color_rejects_invalid_values() {
        ...
        assert_eq!(parse_hex_color("#zzzzzz"), Err("expected only hex digits"));
    }

    #[test]
    fn prompt_background_override_wins_over_terminal_background() {
        assert_eq!(
            set_user_message_bg_override(Some("#112233".to_string())),
            None
        );
        assert_eq!(
            user_message_style_for(Some((255, 255, 255))).bg,
            Some(Color::Rgb(0x11, 0x22, 0x33))
        );
        assert_eq!(set_user_message_bg_override(/*value*/ None), None);
    }
}
